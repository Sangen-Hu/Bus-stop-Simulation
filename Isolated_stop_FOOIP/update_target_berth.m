function [berth_No,newbus,newberth] = update_target_berth(bus_No,bus,berth,bus_location,bus_lane_list,berth_location,berth_num,lag_delay,thres_overtaking_in)
    ...
    if bus(bus_No).berthNo == 0
        if bus(bus_No).pro_overtaking_in <= thres_overtaking_in  
            for i = 1: berth_num
                if i ~= berth_num  
                    if (berth(i).current_bus == 0 || berth(i).current_bus == bus_No) && (bus_location(bus_No) <= berth_location(i)) && (berth(i+1).potential_bus == 0 || bus_location(berth(i+1).potential_bus) <= bus_location(bus_No)-4) 
                     ...
                     if bus_location(bus_No) < berth_location(berth_num)
