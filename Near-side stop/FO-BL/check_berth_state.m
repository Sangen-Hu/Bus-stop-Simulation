            ...
            if berth(k).current_bus == 0
                if k == 1
                    ...
                    if berth(k-1).current_bus ~= 0
                        for j = 1: length(bus_list)
                            if bus_location(bus_list(j)) <= berth_location(k) && bus_location(bus_list(j)) > berth_location(k) -4
                                if bus(bus_list(j)).mission == 3 && bus(bus_list(j)).pro_overtaking_out <= thres_overtaking_out && is_possible_overtaking_out(bus_list(j),adjacent_lane_list,bus_location) && length(buffer_lane_list) < buffer_num && berth(k).block_out == 0
            ...
            if berth(k).current_bus == 0
                if k == 1
                    ...
                    if berth(k-1).current_bus ~= 0
                        for j = 1: length(bus_list)
                            if bus_location(bus_list(j)) <= berth_location(k) && bus_location(bus_list(j)) > berth_location(k) -4
                                if bus(bus_list(j)).mission == 3 && bus(bus_list(j)).pro_overtaking_out <= thres_overtaking_out && is_possible_overtaking_out(bus_list(j),adjacent_lane_list,bus_location) && berth(k).block_out == 0 ...
