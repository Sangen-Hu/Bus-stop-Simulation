function [newbus,newberth,newbus_location,newblock_berth_No] = move_forward(bus_No,bus,berth,bus_location,berth_location,buffer_location,run_step,time_step,berth_num,back_headway,block_berth_No,buffer_num,bus_running_list)
    ...
    if bus(bus_No).reaction_time > 0
        ...
        for k = 1:berth_num
            if berth(k).current_bus == bus_No && bus_location(bus_No) == berth_location(k)
                ...
                if berth(k).block_position == 1
                    [bus,berth] = clear_berth_block(k,bus,berth);
                    berth(k).block_position = 0;
                    if block_berth_No == k
                        block_berth_No = 0;
                    end
                end  
        ...
        if buffer_num ~= 0 && bus_location(bus_No) == buffer_location(buffer_num) 
            if berth(1).current_bus ~= 0 && bus(berth(1).current_bus).speed == 0 && bus(berth(1).current_bus).current_lane == 1
        ...
        if bus_location(bus_No) >= berth_location(1) && bus(bus_No).leave_stop_time == 0
           bus(bus_No).leave_stop_time = run_step;
        end
    end
    newbus = bus;
    newberth = berth;
