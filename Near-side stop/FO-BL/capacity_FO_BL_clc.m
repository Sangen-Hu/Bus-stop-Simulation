function [bus_stop_capacity,average_delay] = capacity_FO_BL_clc(berth_num_in,buffer_num_in,dist_num_in,cycle_length_in,service_cs_in,overtaking_in,overtaking_out)
global buffer_num;
global berth_num;
global back_headway;
...
berth = struct('block_position',{},'block_in',{},'block_out',{},'current_bus',{},'potential_bus',{}); 
...
for count = 1: berth_num
    ...
    berth(count).block_position = 0;
    berth(count).block_in = 0;
    berth(count).block_out = 0;
end
    ...
    for i = 1: length(bus_running_list)
        if i <= length(bus_running_list)
                ...
                if bus(bus_running_list(i)).lane_No == 2  
                    if bus_location(bus_running_list(i)) < berth_location(bus(bus_running_list(i)).target_berth)
                        ...
                        if is_possible_procede(bus_running_list(i),adjacent_lane_list,bus,berth,bus_location,berth_location) 
                            [bus,berth,bus_location,block_berth_No] = move_forward(bus_running_list(i),bus,berth,bus_location,berth_location,buffer_location,run_step,time_step,berth_num,back_headway,block_berth_No,buffer_num,bus_running_list);
                        else
                            bus(bus_running_list(i)).reaction_time = back_headway; 
                            bus(bus_running_list(i)).speed = 0;
                        end
                        if bus_location(bus_running_list(i)) > berth_location(bus(bus_running_list(i)).target_berth+1) 
                            berth(bus(bus_running_list(i)).target_berth).current_bus = bus_running_list(i);  
                            if berth(bus(bus_running_list(i)).target_berth).block_position ~= 1 && berth(bus(bus_running_list(i)).target_berth+1).current_bus ~= 0 && isempty(find(bus_lane_list == berth(bus(bus_running_list(i)).target_berth+1).current_bus)) ~= 1 ...
                                && bus_location(berth(bus(bus_running_list(i)).target_berth+1).current_bus)...
                                == berth_location(bus(bus_running_list(i)).target_berth+1)
                                berth(bus(bus_running_list(i)).target_berth).block_position = 1;
                                berth = update_berth_block(bus(bus_running_list(i)).target_berth,berth);
                                if block_berth_No == 0
                                    block_berth_No = bus(bus_running_list(i)).target_berth;
                                else
                                    if bus(berth(block_berth_No).current_bus).service_time <= bus(bus_running_list(i)).service_time
                                        block_berth_No = bus(bus_running_list(i)).target_berth;
                                    end
                                end    
                            end
                        ...
                        if bus_location(bus_running_list(i)) == berth_location(bus(bus_running_list(i)).target_berth) 
                            if is_possible_overtaking_in(bus_running_list(i),bus_lane_list,bus_location)
                               ...
                               [bus,berth,block_berth_No] = check_arrive_berth(bus_running_list(i),bus,berth,bus_location,berth_location,run_step,berth_num,block_berth_No);
                                %------ block berth------%
                                if bus(bus_running_list(i)).berthNo ~= 0  && berth(bus(bus_running_list(i)).berthNo).block_position ~= 1
                                    if berth(bus(bus_running_list(i)).berthNo+1).current_bus ~= 0 && isempty(find(bus_lane_list == berth(bus(bus_running_list(i)).berthNo+1).current_bus)) ~= 1 ...
                                        && bus_location(berth(bus(bus_running_list(i)).berthNo+1).current_bus)...
                                        == berth_location(bus(bus_running_list(i)).berthNo+1)
                                        berth(bus(bus_running_list(i)).berthNo).block_position = 1;
                                        berth = update_berth_block(bus(bus_running_list(i)).berthNo,berth);
                                        if block_berth_No == 0
                                            block_berth_No = bus(bus_running_list(i)).berthNo;
                                        else
                                            if bus(berth(block_berth_No).current_bus).service_time <= bus(bus_running_list(i)).service_time
                                                block_berth_No = bus(bus_running_list(i)).berthNo;
                                            end
                    ...
                    if bus(bus_running_list(i)).current_lane ~= bus(bus_running_list(i)).lane_No
                        if bus_location(bus_running_list(i)) == berth_location(bus(bus_running_list(i)).target_berth)+1
                            bus_location(bus_running_list(i)) = berth_location(bus(bus_running_list(i)).target_berth);
                        end    
                    ...
                    if bus(bus_running_list(i)).current_lane == 1
                        ...
                        if bus_location(bus_running_list(i)) > berth_location(bus(bus_running_list(i)).target_berth)-4 && berth(bus(bus_running_list(i)).target_berth).current_bus == 0
                            berth(bus(bus_running_list(i)).target_berth).current_bus = bus_running_list(i);  %lock the target berth
                        end   
                        if bus_location(bus_running_list(i)) == berth_location(bus(bus_running_list(i)).target_berth)
                            [bus,berth,block_berth_No] = check_arrive_berth(bus_running_list(i),bus,berth,bus_location,berth_location,run_step,berth_num,block_berth_No);
                            bus_location_plot(bus_running_list(i),run_step-bus(bus_running_list(i)).start_run+1) = bus_location(bus_running_list(i));
            ...
            if bus(bus_running_list(i)).mission == 3
                ...
                if bus(bus_running_list(i)).current_lane == 1
                     for k = 1:berth_num
                        if bus_location(bus_running_list(i)) == berth_location(k)
                            current_berth = k;
                            break;
                        end
                     end 
                ...
                if bus(bus_running_list(i)).current_lane == 1  % bus在 lane-1，车道
                    if bus(bus_running_list(i)).pro_overtaking_out <= thres_overtaking_out
                       if bus(bus_running_list(i)).reaction_time <= 0 && is_possible_procede(bus_running_list(i),bus_lane_list,bus,berth,bus_location,berth_location) 
                           ...
                           if buffer_num ~= 0 && isempty(find(buffer_lane_list == bus_running_list(i), 1)) && (isempty(buffer_lane_list) == 1 || (length(buffer_lane_list) < buffer_num && bus_location(buffer_lane_list(end)) ~= buffer_location(buffer_num))) 
                           ...
                           if current_berth ~= 0 && berth(current_berth).block_out ~= 0 % 如果这辆车被block住了
                               bus(bus_running_list(i)).speed = 0;
                               ...
                               if buffer_num ~= 0
                                   if isempty(buffer_lane_list) == 0 && isempty(find(buffer_lane_list == bus_running_list(i), 1)) && (length(buffer_lane_list)== buffer_num || bus_location(buffer_lane_list(end)) == buffer_location(buffer_num))%2019-10-14更改
                                       bus(bus_running_list(i)).speed = 0;
                                       bus(bus_running_list(i)).reaction_time = bus(bus_running_list(i)).reaction_time - time_step;  %反应时间倒计时
            ...
            if bus(bus_running_list(i)).mission == 4
               if buffer_num == 0 
                   ...
                   if bus(bus_running_list(i)).current_lane ~= 3 
                       if isempty(buffer_lane_list) == 1 || (length(buffer_lane_list) == buffer_num && bus_location(buffer_lane_list(end)) > buffer_location(buffer_num))...%2019-10-14更改：以buffer最上游车位置来判断是否以动，所以删掉下面的判断条件
                ...
                if bus(bus_running_list(i)).reaction_time > 0
                    ...
                    if buffer_num == 0
                       if berth(1).current_bus == bus_running_list(i)
                           ...
                           if berth(1).block_position == 1 
                               [bus,berth] = clear_berth_block(1,bus,berth); 
                               berth(1).block_position = 0;
                               if block_berth_No == 1
                                   block_berth_No = 0;
                               end
                           end
