function [bus_stop_capacity,average_delay] = capacity_FO_BB_clc(berth_num_in,buffer_num_in,dist_num_in,cycle_length_in,service_cs_in,overtaking_in,overtaking_out)
global buffer_num;
global berth_num;
global back_headway;
        ...
        if i <= length(bus_running_list)
                               ...
                               [bus,berth] = check_arrive_berth(bus_running_list(i),bus,berth,bus_location,berth_location);
                                if bus(bus_running_list(i)).berthNo ~= 0 
                                    if berth(bus(bus_running_list(i)).berthNo+1).current_bus ~= 0 && isempty(find(bus_lane_list == berth(bus(bus_running_list(i)).berthNo+1).current_bus)) ~= 1 ...
                                        ...
                                        if bus(bus_running_list(i)).pro_overtaking_out <= thres_overtaking_out && bus(berth(bus(bus_running_list(i)).berthNo+1).current_bus).mission == 2
                ...
                if bus(bus_running_list(i)).current_lane == 1  
                    if bus(bus_running_list(i)).pro_overtaking_out <= thres_overtaking_out
                       if bus(bus_running_list(i)).reaction_time <= 0 && is_possible_procede(bus_running_list(i),bus_lane_list,bus,berth,bus_location,berth_location) 
                           ...
                           if current_berth ~= 0 && berth(current_berth).block == 1 
                               bus(bus_running_list(i)).speed = 0;
                               if isempty(find(buffer_lane_list == bus_running_list(i))) == 0  
                                   buffer_lane_list(buffer_lane_list == bus_running_list(i)) = []; 
                               end
                           else
                               if buffer_num ~= 0
                                   if isempty(buffer_lane_list) == 0 && isempty(find(buffer_lane_list == bus_running_list(i))) && (length(buffer_lane_list)== buffer_num || bus_location(buffer_lane_list(end)) == buffer_location(buffer_num))
                                       bus(bus_running_list(i)).speed = 0;
                                       bus(bus_running_list(i)).reaction_time = bus(bus_running_list(i)).reaction_time - time_step; 
            ...
            if bus(bus_running_list(i)).mission == 4
               if buffer_num == 0 
                   ...
                   if bus(bus_running_list(i)).current_lane ~= 3 % lane-3:buffer lane
