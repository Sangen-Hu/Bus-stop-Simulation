% clearvars -except bus_service_time
% global bus;
% global berth;
global berth_num;
global back_headway;
...
%-----定义公交车位属性的结构体-----%
%------block:车位是否被相邻车block
%------current_bus:berth当前所停靠的车
%------potential_bus:berth预分配给服务的车
berth = struct('block',{},'current_bus',{},'potential_bus',{}); 

% -------simulation parameters------------%
% -------1. bus initiate parameters-------%
bus_num = 30;        % simulation bus number
...
bus_service_variation = 0; % namely cs
...
% -------2. rule set-------%
thres_overtaking_in = 1;    %超车进入下游空余车位的概率阈值
                ...
                for k = 2: berth_num
                    if berth(k).current_bus == 0
                        if berth(k-1).current_bus ~= 0
                            for j = 1: length(bus_lane_list)
                                if bus_location(bus_lane_list(j)) <= berth_location(k) && bus_location(bus_lane_list(j)) > berth_location(k) -4
                                    if bus(bus_lane_list(j)).mission == 3 && bus(bus_lane_list(j)).pro_overtaking_out <= thres_overtaking_out && is_possible_overtaking_out(bus_lane_list(j),adjacent_lane_list,bus_location) && berth(k).block == 0

                                    else
                                        berth(k).current_bus = bus_lane_list(j);
                                    end
                                    break;
                                end
                            end
                        ...
                        if bus_location(bus_running_list(i)) == berth_location(bus(bus_running_list(i)).target_berth) %(bus(bus_running_list(i)).speed == 0 || bus_location(bus_running_list(i),run_step) == berth_location(1))
                            if is_possible_overtaking_in(bus_running_list(i),bus_lane_list,bus_location)
                               ...
                               [bus,berth] = check_arrive_berth(bus_running_list(i),bus,berth,bus_location,berth_location);
                                %------ block berth------%
                                if bus(bus_running_list(i)).berthNo ~= 0 % 说明换道进来了,且相邻车道有车，则该车被block住
                                    if berth(bus(bus_running_list(i)).berthNo+1).current_bus ~= 0 && isempty(find(bus_lane_list == berth(bus(bus_running_list(i)).berthNo+1).current_bus)) ~= 1 ...
                                        ...
                                        if bus(bus_running_list(i)).pro_overtaking_out <= thres_overtaking_out && bus(bus_running_list(i)).mission 
                                            berth(bus(bus_running_list(i)).berthNo+1).block = 1;
                                        end
                                    end
                                end
                               %------ block berth end------%
...
%                 [bus,berth] = bus_servicing(bus_running_list(i),bus,berth,bus_location,berth_location);
            ...
            if bus(bus_running_list(i)).mission == 3 % 首先判断该辆车的任务 如果为3：表示出站
                 ...
                 if bus(bus_running_list(i)).current_lane == 1  % bus在 lane-1，车道
                    if bus(bus_running_list(i)).pro_overtaking_out <= thres_overtaking_out  %如果车可以超车出去，如果此时没有反应时间且顺序可以出站，则先顺序出站
                        if bus(bus_running_list(i)).reaction_time <= 0 && is_possible_procede(bus_running_list(i),bus_lane_list,bus,berth,bus_location,berth_location)
                           ...
                           if current_berth ~= 0 && berth(current_berth).block == 1 % 如果这辆车被block住了
                               bus(bus_running_list(i)).speed = 0;
                           else
                ...
                if bus_location(bus_running_list(i)) == berth_location(1) % bus到达信号灯处，任务改为4 判断离开
                    bus(bus_running_list(i)).mission = 4;
                    bus_location_plot(bus_running_list(i),run_step-bus(bus_running_list(i)).start_run+1) = bus_location(bus_running_list(i));
                    continue;
                end
