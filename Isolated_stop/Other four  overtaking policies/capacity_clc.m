...
% -------2. rule set-------%
thres_overtaking_in = overtaking_in;    %超车进入下游空余车位的概率阈值
thres_overtaking_out = overtaking_out;   %上游bus超车驶出车位的概率阈值
...
for count = 1: berth_num
    berth_location(count) = 4*(berth_num-count+1);
% berth的状态初始化  
...
while 1
    ...
    if isempty(bus_running_list) ~= 1 && bus_running_list(end) == 300
    end
    if isempty(bus_queue_list) ~= 1
