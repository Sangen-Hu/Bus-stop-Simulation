...
%     global bus_location;
    ...
    for i = 1:length(bus_list)-1   
        if (bus_location(bus_list(length(bus_list)))) >= bus_location(bus_No)  % case1:数组最后一个bus位置大于换进来的bus位置 加了==
        ...
        if bus_location(bus_list(1)) <= bus_location(bus_No) % case1:数组第一个bus都小于插入的bus的位置，可以换道进来
...
%     if isempty(renew_list)
%         bus(bus_No).reaction_time = back_headway; %hsg 
%         return;
%     end
end
