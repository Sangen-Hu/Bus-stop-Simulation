function [bus_stop_capacity,average_delay] = capacity_FO_BL_clc(berth_num_in,service_cs_in,overtaking_in,overtaking_out)
...
bus_num = 30000;        % simulation bus number
bus_flow_rate = 36000;
...
average_delay = 0;
last_bus_endtime = 0;
bus_service_time = zeros(1,bus_num);
...
adjacent_lane_list = [];
bus_queue_list = [];
unit_movlength = 4; % 以moveup_headway时间为一个berth长度，为精细化，将berth细分多个单元
delate_arry = [];
bus(1).start_run = 1;
current_berth = 0;
block_berth_No = 0;
...
for n = 1:bus_num
    ...
    if bus_service_variation ~= 0
        bus_service_time(n) = gamrnd(bus_service_alpha,bus_service_beta); % pre-generate all buses service time
    else
        bus_service_time(n) = unifrnd (bus_service_meantime,bus_service_meantime);
    end
        ...
        if i <= length(bus_running_list)
            ...
            if bus(bus_running_list(i)).mission == 1 
               if bus(bus_running_list(i)).target_berth == 0 || bus(bus_running_list(i)).lane_No == 2
                   [temp_berth_assigned,bus,berth] = update_target_berth(bus_running_list(i),bus,berth,bus_location,bus_lane_list,berth_location,berth_num,lag_delay,block_berth_No,thres_overtaking_in);
                ...
                if bus(bus_running_list(i)).lane_No == 2  
                    if bus_location(bus_running_list(i)) < berth_location(bus(bus_running_list(i)).target_berth)
                        ...
                        if is_possible_procede(bus_running_list(i),adjacent_lane_list,bus,berth,bus_location,berth_location) 
                            [bus,berth,bus_location,block_berth_No] = move_forward(bus_running_list(i),bus,berth,bus_location,berth_location,berth_num,block_berth_No,bus_running_list);
                        else
                            bus(bus_running_list(i)).reaction_time = back_headway; 
                            bus(bus_running_list(i)).speed = 0;
                        end
                 ...
                 if bus(bus_running_list(i)).current_lane == 1  % bus在 lane-1，车道
                    if bus(bus_running_list(i)).pro_overtaking_out <= thres_overtaking_out  
                        if bus(bus_running_list(i)).reaction_time <= 0 && is_possible_procede(bus_running_list(i),bus_lane_list,bus,berth,bus_location,berth_location)
                           ...
                           if current_berth ~= 0 && berth(current_berth).block_out ~= 0 
                               bus(bus_running_list(i)).speed = 0;
                           else
...
bus_stop_capacity = (bus_num-warm_up_bus) / ((last_bus_endtime - warm_up_endtime)/3600);
