            ...
            if bus(bus_running_list(i)).mission == 0
                if bus(bus_running_list(i)).lane_No == 0
                    if is_possible_leavesignal(green_time,cycle_length,current_time) 
                       available_berth_num = check_vacant_berth(bus,berth,berth_num);
                       if buffer_num ~= 0 && (((isempty(buffer_lane_list) ~=1 && isempty(find(bus_location(buffer_lane_list) == buffer_location(1))) ~=1 && bus(buffer_lane_list(bus_location(buffer_lane_list) == buffer_location(1))).speed == 0 && length(buffer_lane_list) >= buffer_num)...
                           ...
                           if buffer_num > 0
                               ...
                               [temp_berth_assigned,bus,berth] = update_target_berth(bus_running_list(i),bus,berth,bus_location,bus_lane_list,berth_location,adjacent_lane_list,berth_num,lag_delay,block_berth_No,thres_overtaking_in,stop_entrance);
                ...
                if bus(bus_running_list(i)).lane_No == 1 && bus(bus_running_list(i)).mission == 0
                    if is_possible_procede(bus_running_list(i),buffer_lane_list,bus,berth,bus_location,berth_location) 
                        [bus,berth,bus_location,buffer_lane_list,block_berth_No] = move_forward(bus_running_list(i),bus,berth,bus_location,berth_location,buffer_location,buffer_lane_list,run_step,time_step,berth_num,back_headway,block_berth_No,bus_running_list,buffer_num);
                    else
                        bus(bus_running_list(i)).reaction_time = back_headway; 
                        bus(bus_running_list(i)).speed = 0;
                    end
                    ...
                    if bus_location(bus_running_list(i)) < berth_location(bus(bus_running_list(i)).target_berth)
                        ...
                        if bus_location(bus_running_list(i)) > berth_location(bus(bus_running_list(i)).target_berth+1) 
                            ...
                            if berth(bus(bus_running_list(i)).target_berth).block_position ~= 1 && berth(bus(bus_running_list(i)).target_berth+1).current_bus ~= 0 && isempty(find(bus_lane_list == berth(bus(bus_running_list(i)).target_berth+1).current_bus)) ~= 1 ...
                                && bus_location(berth(bus(bus_running_list(i)).target_berth+1).current_bus) == berth_location(bus(bus_running_list(i)).target_berth+1)
                               ...
                               [bus,berth] = check_arrive_berth(bus_running_list(i),bus,berth,bus_location,berth_location);
                                ...
                                if bus(bus_running_list(i)).berthNo ~= 0  && berth(bus(bus_running_list(i)).berthNo).block_position ~= 1
                                    if berth(bus(bus_running_list(i)).berthNo+1).current_bus ~= 0 && isempty(find(bus_lane_list == berth(bus(bus_running_list(i)).berthNo+1).current_bus)) ~= 1 ...
                                        && bus_location(berth(bus(bus_running_list(i)).berthNo+1).current_bus) == berth_location(bus(bus_running_list(i)).berthNo+1) && berth(bus(bus_running_list(i)).berthNo).block_position == 0 
                           ...
                           if is_possible_overtaking_in(bus_running_list(i),bus_lane_list,bus_location) 
                               if bus(bus_running_list(i)).reaction_time >= 0
                                   ...
                                   if bus(bus_running_list(i)).reaction_time <= 0
                                       ...
                                       [bus,berth] = check_arrive_berth(bus_running_list(i),bus,berth,bus_location,berth_location);
                                   %------ block berth------%
                                        if bus(bus_running_list(i)).berthNo ~= 0
                                            if berth(bus(bus_running_list(i)).berthNo+1).current_bus ~= 0 && isempty(find(bus_lane_list == berth(bus(bus_running_list(i)).berthNo+1).current_bus)) ~= 1 ...
                                                && bus_location(berth(bus(bus_running_list(i)).berthNo+1).current_bus) == berth_location(bus(bus_running_list(i)).berthNo+1) && berth(bus(bus_running_list(i)).berthNo).block_position == 0 
                                                berth(bus(bus_running_list(i)).berthNo).block_position = 1;
                                                berth = update_berth_block(bus(bus_running_list(i)).berthNo,berth);
