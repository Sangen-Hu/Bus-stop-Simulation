function [berth_No,newbus,newberth] = update_target_berth(bus_No,bus,berth,bus_location,bus_lane_list,berth_location,adjacent_lane_list,berth_num,lag_delay,block_berth_No,thres_overtaking_in,stop_entrance)

	berth_No = 0;
	if bus(bus_No).berthNo == 0
        if bus(bus_No).pro_overtaking_in <= thres_overtaking_in  
            for i = 1: berth_num
                if (berth(i).current_bus == 0 || berth(i).current_bus == bus_No) && (bus_location(bus_No) <= berth_location(i)+1)
                    if block_berth_No ~= 0 && bus_location(bus_No) <= berth_location(block_berth_No)-4 && berth(i).block_in ~= 0
                        continue;
                    end
